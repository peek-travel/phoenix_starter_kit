...
- **Never** use `# coveralls-ignore-start` / `# coveralls-ignore-end` to skip lines. If code appears unreachable, ask why it exists — if code is so defensive that it can't be tested, it should be removed, not hidden. Use `coveralls.json` `skip_files` only for generated or infrastructure files already listed there.
